          ...
          {t.steps.map((step: any, index: number) => (
            <div key={step.id} className="flex items-center">
              ...
              {index < t.steps.length - 1 && (
                <div
                  className={cn(
                    "bg-muted-foreground/20 relative overflow-hidden mx-2",
                    isMobile ? "w-6 h-0.5" : "w-12 h-0.5 md:w-16",
                  )}
                >
                  <motion.div
      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-center p-4 md:p-6 lg:p-8">
        ...
        >
          <div className="flex items-center space-x-3 md:space-x-4">
            <motion.div
              className={cn(
                "bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 rounded-2xl flex items-center justify-center shadow-2xl",
                isMobile ? "w-12 h-12" : "w-14 h-14 md:w-16 md:h-16",
              )}
              whileHover={{ scale: 1.05, rotate: 5 }}
              transition={{ type: "spring", stiffness: 300 }}
            >
              <Bot className={cn("text-white", isMobile ? "w-6 h-6" : "w-7 h-7 md:w-8 md:h-8")} />
            </motion.div>
            <h1
              className={cn(
        ...
        >
          <Card className="backdrop-blur-xl bg-card/90 border-border/50 shadow-2xl min-h-[500px] relative overflow-hidden">
